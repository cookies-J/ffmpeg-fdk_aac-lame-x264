...
function CompileExternal() {
    mkdir -p "$CURRENT_FOLDER/ffmpeg_external/build_source"
    
    function fdk() {
        if [ ! -r $BUILDED_PATH/$FDK_AAC ]
    then
        ./ffmpeg_external/build-fdk-aac.sh
    fi
    }
    
    function lame() {
        if [ ! -r $BUILDED_PATH/$LAME ]
    then
        ./ffmpeg_external/build-lame.sh
    fi
    }
    function x264() {
        if [ ! -r $BUILDED_PATH/$X264 ]
    then
        ./ffmpeg_external/build-x264.sh
    fi
    }
    function x265() {
        if [ ! -r $BUILDED_PATH/$X265 ]
    then
        ./ffmpeg_external/build-x265.sh
    fi
    }
...
function CleanFFmpegBuildFile() {
    rm -rf $BUILD_THIN_FOLDER $BUILD_FOLDER $SCRATCH $OUTPUT_FOLDER $OUTPUT_FOLDER
}
